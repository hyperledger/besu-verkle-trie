 ...
 * remove nodes in the Verkle Trie
...
public class RemoveVisitor<V> implements PathNodeVisitor<V> {
    ...
    /**
     * Merges a single branching node by updating and replacing its structure with
     * the new node.
     *
    ...
    /**
     * Visits a branch node to remove a node associated with the provided path and
     * maintain the Trie's structure.
     *
    ...
    /**
     * Checks if the branch node should be flattened (merged with its only child)
     * and performs the flattening operation.
     *
     * @param branchNode The branch node to consider for flattening.
     * @return The updated node after flattening or the original branch node if
     *         flattening is not applicable.
     */
    protected Node<V> maybeFlatten(BranchNode<V> branchNode) {
        ...
        final Bytes completePath = Bytes.concatenate(branchNode.getPath(), Bytes.of(onlyChildIndex.get()),
                onlyChild.getPath());
     ...
     *         optional if there is no or more than one non-null child.
     */
